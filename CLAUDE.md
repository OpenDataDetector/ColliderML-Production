...
**Workaround (implemented in `setup_container_env.sh`):**

The `compile.py --pythia8_makefile` approach doesn't work out of the box because the
Pythia8 spack build wasn't compiled with HepMC2 support (no `-lHepMC` in its Makefile.inc).
The workaround is to compile the interface directly with explicit HepMC2 flags:

```bash
# Clone interface source (on host, where network is available):
git clone https://github.com/mg5amcnlo/MG5aMC_PY8_interface.git .cache/MG5aMC_PY8_interface

# Compile inside container (done automatically by setup_container_env.sh):
...
This compiles successfully against Pythia 8.313 + HepMC2 (only deprecation warnings).
The compiled binary is cached in `.cache/MG5aMC_PY8_interface/`.
