...
\begin{itemize}\itemsep0pt
  \item 68\% cluster pitch $< 11\,\mu$m\\
    $\Rightarrow$ 68\% $|\delta_x| < 0.3$\,mm.
  \item Cluster pitch RMS = 0.46\,mm (PU tail)\\
    $\Rightarrow$ $|\delta_x|$ RMS $\approx 11$\,mm.
  \item Worst pitch error 5--10\,mm\\
    $\Rightarrow$ $|\delta_x| > 100$\,mm.
\end{itemize}

The observed 54\,mm barrel RMS is the long PU-merge tail of cluster errors, amplified $\times 25$.
\end{column}
\end{columns}
\end{frame}
...
\footnotesize
\textbf{Both have $\theta=40$\,mrad and the same $\times 25$ amplification.}
But endcap $\sigma_r=27$\,mm vs barrel $\sigma_z=54$\,mm. The \emph{cluster noise input} is different:
\begin{itemize}\itemsep0pt
  \item Each barrel wafer covers the full $\eta$ range $\Rightarrow$ many particles per wafer in PU.
  \item Each endcap wafer covers a narrower angular slice $\Rightarrow$ fewer particles per wafer.
  \item PU multi-particle merging $\Rightarrow$ cluster centroid mm-scale tail $\Rightarrow$ amplified to cm-scale SP$z$.
\end{itemize}

\medskip
\textbf{What would actually fix it:}
\begin{enumerate}\itemsep0pt
  \item \alert{Reduce cluster pitch noise} — better digi clusterer (shape-aware splitting, NN per-particle splitting). Upstream work.
  \item \alert{Larger stereo angle} — detector design; the $1/\sin\theta$ factor shrinks.
  \item \alert{For ML downstream now:} use cluster wafer-centre $z$ in barrel, SP $r$ in endcap. The wafer-centre is bounded by the half-strip and doesn't amplify cluster noise.
\end{enumerate}

\medskip
\alert{\textbf{Bottom line:}} SP construction is geometrically tight in endcap, unavoidably noise-amplifying in barrel. The fix is \emph{cluster reco}, not the SP algorithm.
\end{frame}
