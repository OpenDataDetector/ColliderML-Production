...
from edm4hep_processor import load_edm4hep_event, process_track_data
...
def convert_tracks(
    ...
    for start_run in tqdm(range(0, num_runs, runs_per_chunk), desc="Processing chunks"):
        ...
        for run_idx in range(start_run, min(start_run + runs_per_chunk, len(run_dirs))):
            ...
            if "tracks_df" not in event:
                print(f"No track data found in run {run_idx}")
                continue
            
            # Process event
            event_df = process_track_data(
                run_idx * run_size,
                event["tracks_df"],
                track_states_df=event.get("track_states_df"),
                track_hits_df=event.get("track_hits_df")
            )
