#!/usr/bin/env python3
"""
Convert EDM4HEP calorimeter data to HDF5 format.
"""
...
from edm4hep_processor import load_edm4hep_event, process_calorimeter_data
...
def convert_calorimeter(
    ...
    for start_run in tqdm(range(0, num_runs, runs_per_chunk), desc="Processing chunks"):
        ...
        for run_idx in range(start_run, min(start_run + runs_per_chunk, len(run_dirs))):
            ...
            if "calo_hits_df" not in event:
                print(f"No calorimeter data found in run {run_idx}")
                continue
            
            # Process event
            event_df = process_calorimeter_data(
                run_idx * run_size,
                event["calo_hits_df"],
                calo_contrib_df=event.get("calo_contrib_df")
            )
...
def main():
    parser = argparse.ArgumentParser(description="Convert EDM4HEP calorimeter data to HDF5")
