...
def _merge_measurements_with_tracker(meas_df: pd.DataFrame, tracker_df: pd.DataFrame,
    ...
    # Fill NaN left by the LEFT merge on (true_x, true_y, true_z) when a
    # measurement has no matching sim hit. Unmatched rows lose their truth
    # association; see CLAUDE.md "KNOWN ISSUE: Digihit Measurement–SimHit Merge".
    ...
    for col in int_fill_cols:
        if col in merged.columns and merged[col].isna().any():
            ...
            logging.warning(
                "DIGIHIT MERGE: %d/%d rows (%.1f%%) have NaN in '%s' — filling with 0",
