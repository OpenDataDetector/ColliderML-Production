...
def setup_acts_reconstruction(input_path, output_dir, config, rnd, logger=None):
    ...
    # (ported from colliderml-production PR #43). After ambiguity resolution the
    # resolved ACTS tracks live under the "tracks" alias; convert them to an
    # edm4hep::Track collection ("ActsTracks") and append it onto the original ddsim
    # "events" frame (calo SimHits + MCParticles) via PodioWriter (inputFrame="events"),
    # so pandora_reco gets tracks + calo in ONE file. The metadata frames PodioWriter
    # drops (<coll>__CellIDEncoding) are restored post-run in main() (else the reco
    # stage's DDCaloDigi fails with "bad optional access"). Bz must match the ODD
    # nominal field (3 T) AND Pandora's field, or reconstructed momentum is scaled.
    if getattr(config, "output_sim_with_tracks", False) and getattr(config, "reco", True):
...
def _restore_metadata_frames(with_tracks, sim, logger=None):
    ...
    Rewrites with_tracks in place via a temp file + rename. (Ported from PR #43.)"""
