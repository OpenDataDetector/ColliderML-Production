...
def validate_merge_smear(run_dir, node_idx, runs_per_node):
    """
    Validation steps for merge_smear stage:
    ...
    for run_id in range(start_run, end_run):
        ...
        required_files = ["merged_events.hepmc3"]
...
def main():
    ...
    logger.info(f"Validating merge_smear stage for node {args.node_idx}")
    logger.info(f"Run directory: {args.run_dir}")
    logger.info(f"Runs per node: {args.runs_per_node}")

    validate_merge_smear(args.run_dir, args.node_idx, args.runs_per_node)
