...
def validate_event_ids_streaming(parquet_files: List[Path], config_name: str) -> bool:
    """
...
def upload_config_direct(
    ...
    try:
        ...
        if not skip_validation:
            if not validate_event_ids_streaming(sorted_files, config_name):
...
def update_readme_configs(
    ...
    """
    Update README.md YAML frontmatter to include config definitions.

    This is required for HuggingFace dataset viewer to show multiple configs.
    Each config points to data/{config_name}/*.parquet files.
    ...
    if dry_run:
        for cfg in configs_to_add:
            logger.info(f"  Would add: {cfg['config_name']} -> data/{cfg['config_name']}/*.parquet")
        return True
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            try:
                ...
                logger.warning("Could not download README.md, creating new one")
                readme_content = ""
        ...
        # Add new configs to YAML
        existing_configs = yaml_data.get('configs', [])
        existing_names = {c.get('config_name') for c in existing_configs if c}
        ...
        for cfg in configs_to_add:
            ...
            if config_name not in existing_names:
                ...
                added += 1
                logger.info(f"  Adding: {config_name}")

        if added == 0:
            logger.info("  No new configs to add")
            return True
        ...
        try:
            api.upload_file(
                ...
                repo_id=repo_id,
                repo_type="dataset",
                commit_message=f"Update configs: add {added} new config(s)"
            )
            logger.info(f"✓ Updated README.md with {added} config(s)")
        finally:
            os.unlink(tmp_path)
  ...
  python upload_to_hf_unified_simple.py unified_dataset_config.yaml --skip-validation
    ...
    try:
        ...
        if not configs_to_upload:
            logger.info("No configs to upload after filtering")
            sys.exit(0)
        ...
        # Update README configs for HF dataset viewer
        if successful_configs and not args.dry_run:
            logger.info(f"\nUpdating README.md with {len(successful_configs)} config(s)...")
            update_readme_configs(repo_id, successful_configs, token, args.dry_run)
