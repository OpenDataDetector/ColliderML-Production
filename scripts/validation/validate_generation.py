...
def validate_generation(run_dir, node_idx, runs_per_node):
    """
    Validation steps for generation stage:
    ...
    for run_id in range(start_run, end_run):
        ...
        required_files = ["events.hepmc3", "events_pileup.hepmc3"]
...
def main():
    ...
    logger.info(f"Validating generation stage for node {args.node_idx}")
    logger.info(f"Run directory: {args.run_dir}")
    logger.info(f"Runs per node: {args.runs_per_node}")

    validate_generation(args.run_dir, args.node_idx, args.runs_per_node)
