...
Table~\ref{tab:crosseval} presents the cross-evaluation matrix for $d_0$.
We evaluate the nominal-trained v9 model on all three datasets.

\begin{table}[h]
\centering
\caption{$d_0$ resolution (IQR$/1.349$, mm) of the nominal-trained transformer (v9) evaluated on different beam spot datasets. The zero baseline predicts $d_0 = 0$ regardless of dataset.}
\label{tab:crosseval}
...
\begin{enumerate}
    ...
    \item \textbf{\SI{25}{\micro\metre} shift}: ML (\SI{0.023}{\milli\metre}) is better than zero (\SI{0.031}{\milli\metre}), suggesting some genuine track fitting emerges when the beam spot prior is partially wrong. CKF is invariant (\SI{0.032}{\milli\metre}).
    \item \textbf{\SI{300}{\micro\metre} shift}: ML (\SI{0.057}{\milli\metre}) $\approx$ CKF (\SI{0.059}{\milli\metre}). The beam spot prior is now wrong by $24\sigma_{xy}$; ML falls back to CKF-level performance from actual track fitting. The zero baseline (\SI{0.279}{\milli\metre}) is catastrophic.
\end{enumerate}
